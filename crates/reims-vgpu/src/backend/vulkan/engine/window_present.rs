...
impl WindowPresenter {
    ...
    pub(crate) unsafe fn present(
        ...
        if retry.attempted {
            self.cadence_acquire_retried = self.cadence_acquire_retried.saturating_add(1);
        }
        if retry.rescued {
            self.cadence_acquire_rescued = self.cadence_acquire_rescued.saturating_add(1);
        }
        ...
        let submit_result = (|| {
            ...
            if let Some((dst_rect, tex_wh)) = cursor_draw {
                ...
                gpu.record(ctx, frame_cmd, framebuffer, self.extent, dst_rect, tex_wh);
            } else {
                image_barrier(
                    &ctx.device,
                    frame_cmd,
